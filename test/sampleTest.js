...
contract('Splitter', function(accounts) {
    ...
    before("should prepare accounts", function() {
        ...
        return web3.eth.makeSureAreUnlocked([ alice, bob, carol ])
            .then(() => web3.eth.makeSureHasAtLeast(alice, [ bob, carol ], web3.utils.toWei("2")))
    ...
    describe("split", function() {
        ...
        it("should reject without Bob", function() {
            return expectedException(
                () => splitter.split(zeroAdd, carol, { from: alice, value: 1000, gas: 3000000 }),
                3000000);
        });

        it("should reject without Carol", function() {
            return expectedException(
                () => splitter.split(bob, zeroAdd, { from: alice, value: 1000, gas: 3000000 }),
                3000000);
        });
        ...
        valueSetSingle.forEach(values => {
            ...
            describe("single split situation: " + values.sent + " -> " + values.bob + " + " + values.carol, function() {
                ...
                it("should have indexed topics in event when split", function() {
                    return splitter.split(bob, carol, { from: alice, value: values.sent })
                        .then(txObject => {
                            assert.strictEqual(txObject.receipt.rawLogs.length, 1);
                            assert.strictEqual(txObject.receipt.rawLogs[0].topics.length, 4);
                            assert.strictEqual(
                                txObject.receipt.rawLogs[0].topics[0],
                                web3.utils.sha3("LogSplit(address,address,address,uint256)"));
                            assert.strictEqual(
                                txObject.receipt.rawLogs[0].topics[1],
                                alice.replace("0x", "0x000000000000000000000000").toLowerCase());
                            assert.strictEqual(
                                txObject.receipt.rawLogs[0].topics[2],
                                bob.replace("0x", "0x000000000000000000000000").toLowerCase());
                            assert.strictEqual(
                                txObject.receipt.rawLogs[0].topics[3],
                                carol.replace("0x", "0x000000000000000000000000").toLowerCase());
                        });
                });
    ...
    describe("withdraw", function() {
        ...
        it("should reject withdraw if value passed", function() {
            return expectedException(
                () => splitter.withdraw({ from: bob, value: 1, gas: 3000000 }),
                3000000);
        });
        ...
        it("should have indexed topics in event when bob withdraws", function() {
            return splitter.withdraw({ from: bob })
                .then(txObject => {
                    assert.strictEqual(txObject.receipt.rawLogs.length, 1);
                    assert.strictEqual(txObject.receipt.rawLogs[0].topics.length, 2);
                    assert.strictEqual(
                        txObject.receipt.rawLogs[0].topics[0],
                        web3.utils.sha3("LogWithdrawn(address,uint256)"));
                    assert.strictEqual(
                        txObject.receipt.rawLogs[0].topics[1],
                        bob.replace("0x", "0x000000000000000000000000").toLowerCase());
                });
        ...
        it("should increase bob balance with amount", function() {
            ...
            return web3.eth.getBalance(bob)
                .then(balance => bobBalanceBefore = toBN(balance))
                .then(() => splitter.withdraw({ from: bob }))
                .then(txObject => web3.eth.getTransaction(txObject.tx)
                        .then(tx => txFee = toBN(tx.gasPrice).mul(toBN(txObject.receipt.gasUsed))))
                .then(() => web3.eth.getBalance(bob))
                .then(balance => assert.strictEqual(
                    bobBalanceBefore.add(toBN(1000)).sub(toBN(txFee)).toString(10),
                    balance.toString(10)));
        });
    ...
    describe("state reconstruction", function() {
        ...
        beforeEach("fetch fromBlock", function() {
            return web3.eth.getTransactionReceipt(splitter.transactionHash)
